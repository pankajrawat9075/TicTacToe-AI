...
def minimax(board):
    ...
    if terminal(board):
        ...
        if player(board) == X:
            value, move = max_value(board,depth,-100,+100)
            return move
        else:
            value, move = min_value(board,depth,-100,+100)
            return move
...
def max_value(board,depth,alpha,beta):
    ...
    for action in actions(board):
        # v = max(v, min_value(result(board, action))) with apha beta pruning and depth optimization
        aux, act = min_value(result(board, action),depth-1,alpha,beta)
        alpha=max(alpha,aux)            
        if aux > v:
            v = aux
            move = action
